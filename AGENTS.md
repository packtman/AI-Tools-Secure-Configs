...
- `.github/workflows/*.yml` — Scheduled discovery and validation workflows
...
```bash
# Validate all deployable config files and workflows
...
- The `.github/workflows/config-discovery.yml` workflow watches upstream sources and opens config-maintenance PRs.
- The `.github/workflows/config-validation.yml` workflow validates config syntax on pull requests.
