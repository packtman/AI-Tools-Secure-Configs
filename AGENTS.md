...
- `.py` — Automation and validation helpers under `automation/` and `scripts/`
...
# Validate all deployable config examples and shell scripts
...
- `.github/workflows/config-discovery.yml` runs scheduled upstream source discovery and opens config-maintenance PRs.
- `.github/workflows/config-validation.yml` validates config examples on pull requests.
