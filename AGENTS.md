...
```bash
# Validate all deployable config examples and JSONC examples
...
- `.github/workflows/config-discovery.yml` runs the automated upstream config discovery loop.
- `.github/workflows/config-validation.yml` validates config syntax on pull requests.
