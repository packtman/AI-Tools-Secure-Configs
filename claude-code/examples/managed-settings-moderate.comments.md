...
**What:** Absolute path to the bubblewrap binary on Linux/WSL2 (managed settings only).

**Why (Moderate tier):** Locked-down developer images often remove broad `PATH` search. Pinning the binary keeps sandboxed Bash working when `bwrap` is installed at a known admin path.

**What breaks if wrong:** Sandbox cannot start. On Moderate, Claude Code continues without sandboxing when `failIfUnavailable` is `false`. Ignored on macOS (Seatbelt). Adjust the path if your distro installs `bwrap` elsewhere (for example `/usr/local/bin/bwrap`).

### `sandbox.socatPath: "/usr/bin/socat"`
**What:** Absolute path to `socat` used by the Linux/WSL2 sandbox network proxy (managed settings only).

**Why (Moderate tier):** Same locked-PATH / non-standard install case as `bwrapPath`.

**What breaks if wrong:** Networked sandboxed commands fail, or the sandbox cannot start. Adjust for your package layout.
...
**What:** When set in Windows admin-managed settings (HKLM or `C:\Program Files\ClaudeCode\managed-settings.json`), Claude Code on WSL also reads the Windows policy chain, with Windows sources taking priority over `/etc/claude-code`.

**Why (Moderate tier):** One Intune/Jamf-style Windows payload covers both native Windows and WSL users.

**What breaks if false on mixed fleets:** WSL sessions miss Windows-deployed managed settings. Not honored from server-managed settings (use MDM or the Windows managed file).

---

## Optional: `processWrapper` / `policyHelper`

Not set in the deployable Moderate JSON by default (wrong paths refuse startups or become the sole managed source).

- `processWrapper`: absolute corporate launcher path for Claude Code self-spawns. Requires Claude Code >= 2.1.210. Equivalent env: `CLAUDE_CODE_PROCESS_WRAPPER` (env wins if both set). Ignored on Windows. Ignored from project/local settings.
- `policyHelper`: MDM-only executable that emits `{ "managedSettings": { ... } }` on stdout. When configured, helper output is the only managed source. Non-zero exit at startup refuses to start. Not honored from server-managed settings.

See `claude-code/examples/corporate-launcher-and-policy-helper.md` for rollout steps, tier delta, and validation.
