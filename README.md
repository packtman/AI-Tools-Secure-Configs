...
Discovery PRs are intended for a config-maintenance agent. The agent reviews the generated report, verifies upstream changes, and updates the affected tiered config or rollout documentation in the same PR when a real admin control changed. If the model credential is unavailable to GitHub Actions, the workflow opens a discovery handoff PR for Cursor Automation or human review instead of failing silently.
