...
Discovery PRs are intended for a config-maintenance agent. The agent reviews the generated report and scoped work list, verifies upstream changes, and updates the affected tiered config or rollout documentation in the same PR when a real admin control changed.

If the model-backed GitHub Actions step is unavailable, the workflow still opens a handoff PR for Cursor Automation or human review instead of dropping the signal. Config edits are validated with [`scripts/validate_config_files.py`](./scripts/validate_config_files.py) before the automation commits them.
