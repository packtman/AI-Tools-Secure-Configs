...
If the scheduled workflow cannot access `ANTHROPIC_API_KEY`, it still opens a discovery handoff PR for Cursor Cloud automation or human review. Config PRs are validated by `scripts/validate_config_files.py` and `.github/workflows/config-validation.yml`.
