...
The scheduled workflow writes each run to an isolated `automation/config-maintenance-<run-id>-<attempt>` branch. If `ANTHROPIC_API_KEY` is unavailable, it still opens a discovery handoff PR for Cursor Automation or human review instead of dropping the signal. Config examples and workflow files are validated by [`scripts/validate_config_files.py`](./scripts/validate_config_files.py) before agent edits are committed and again through the Config Validation workflow on pull requests.
