...
Discovery PRs are intended for a config-maintenance agent. When `ANTHROPIC_API_KEY` is configured, the workflow can run that agent directly. If the secret is unavailable, the workflow opens a discovery-only handoff PR for Cursor Automation or human review. In either path, the reviewer verifies upstream changes and updates affected tiered configs or rollout documentation when a real admin control changed.
