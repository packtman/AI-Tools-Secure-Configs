...
Discovery PRs are intended for a config-maintenance agent. The agent reviews the generated report, verifies upstream changes, and updates the affected tiered config or rollout documentation in the same PR when a real admin control changed. If the GitHub workflow cannot run the agent because `ANTHROPIC_API_KEY` is unavailable, it still opens a discovery-only handoff PR for Cursor Automation or human review.

Every PR can run [`scripts/validate_config_files.py`](./scripts/validate_config_files.py) through the config validation workflow to catch malformed JSON, YAML, TOML, or shell examples before merge.
