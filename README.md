...
Discovery PRs are intended for a config-maintenance agent. The GitHub workflow can run the Claude Code action when `ANTHROPIC_API_KEY` is configured, and otherwise opens a discovery-only handoff PR for Cursor Automation or human review. The agent reviews the generated report, verifies upstream changes, validates edited configs, and updates the affected tiered config or rollout documentation in the same PR when a real admin control changed.
