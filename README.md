...
The GitHub workflow can run the maintenance agent when `ANTHROPIC_API_KEY` is configured. If that key is unavailable, it still opens a discovery-only PR so a Cursor Cloud automation or human reviewer can finish the rollout-engineering update. Deployable config syntax is checked by `scripts/validate_config_files.py` and the config validation workflow.
