...
Discovery PRs are intended for a config-maintenance agent. When the workflow has access to `ANTHROPIC_API_KEY`, it runs the agent, validates edited config files, and commits focused tiered config or rollout documentation updates in the same PR. If the key is not configured, the workflow still opens a report-only PR for Cursor Cloud automation or human review.
