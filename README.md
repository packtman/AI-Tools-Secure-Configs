...
Discovery PRs are intended for a config-maintenance agent. The GitHub workflow runs the agent when `ANTHROPIC_API_KEY` is available; if not, it still opens a handoff PR for Cursor Automation or a human reviewer. The agent reviews the generated report, verifies upstream changes, validates edited config files, and updates the affected tiered config or rollout documentation in the same PR when a real admin control changed.
