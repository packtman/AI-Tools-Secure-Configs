...
| `validate_repo_configs.py` | Validates deployable JSON, YAML, TOML, and shell examples before agent edits are committed. |
...
6. The workflow commits those files to `automation/config-maintenance` and opens or updates the PR branch. If the branch already exists, the workflow continues from that branch so previous agent edits are preserved.
...
8. The agent reads the report, checks the upstream source, updates affected tiered configs and rollout docs, validates the files, and leaves the final edits for the workflow to commit.
9. If the secret is not configured, the workflow still opens a discovery intake PR. A human reviewer or Cursor Cloud automation can then run `agent-prompt.md` on that branch.

GitHub Actions alone can detect and stage the source-change signal. It cannot safely decide the security posture for a brand-new vendor setting without an AI review step. Configure `ANTHROPIC_API_KEY` for the built-in Claude Code action, or use the Cursor Cloud automation prompt in this directory for the final config-update PR behavior.
...
Validate deployable examples after agent edits:

```bash
python3 automation/config-discovery/validate_repo_configs.py
```
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step requires repository secret `ANTHROPIC_API_KEY`. If the secret is missing, the workflow skips the agent and opens a discovery intake PR instead of failing before PR creation.
...
- Groups multiple changed sources for the same tool into one scoped item.
...
- Validates deployable JSON, YAML, TOML, and shell files before committing agent edits.
- Fails the job only when the agent errors and leaves no diff, or when edited deployable files are malformed.
