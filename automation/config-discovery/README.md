...
| `.github/workflows/config-validation.yml` | Pull request validation for deployable config syntax and discovery registry integrity. |
| `scripts/validate_config_files.py` | Shared validator for JSON, YAML, TOML, and shell config examples. |
...
6. The workflow commits those files to a run-specific branch named `automation/config-maintenance-<run_id>-<run_attempt>`.
7. If repository secret `ANTHROPIC_API_KEY` is available, the GitHub workflow runs the config-maintenance agent against `reports/agent-scope.md`.
8. If the secret is missing, the workflow still opens a discovery handoff PR. Use `CURSOR-AUTOMATION.md` or a human review to finish the config update.
9. The maintenance agent reads the report, checks the upstream source, updates affected tiered configs and rollout docs, validates the files, commits the real config changes, and pushes the PR branch.

GitHub Actions alone can detect and stage the source-change signal. It cannot safely decide the security posture for a brand-new vendor setting without an AI review step. Use the Cursor Cloud automation prompt in this directory when the GitHub-hosted agent is unavailable, or when you want Cursor Cloud to own the final config-update PR behavior.

## PR Behavior

- Discovery-only PRs are expected when upstream documentation changed but no model review was available.
- Config-update PRs should include tiered config edits, rationale updates, rollout impact, deployment steps, audit logging notes, rollback notes, and workflow-preservation guidance when behavior changes.
- Each run uses a new maintenance branch so two scheduled runs do not overwrite each other.
- Pull requests run `.github/workflows/config-validation.yml`, which validates deployable JSON, YAML, TOML, shell hooks, and the discovery registry.
...
Validate deployable config syntax:
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step requires repository secret `ANTHROPIC_API_KEY`. If the secret is not configured, the workflow opens a handoff PR instead of failing before reviewers can see the upstream change.
