...
| `scripts/validate_config_files.py` | Shared validator for deployable JSON, YAML, TOML, and shell files. |
| `.github/workflows/config-discovery.yml` | Scheduled workflow that runs the scanner and opens or updates a PR. |
| `.github/workflows/config-validation.yml` | Pull request workflow that validates config syntax. |
...
7. If repository secret `ANTHROPIC_API_KEY` is available, the workflow runs the config-maintenance agent on the scoped report, validates edited config files, commits the real config changes, and opens or updates the PR.
8. If `ANTHROPIC_API_KEY` is unavailable, the workflow still opens a discovery-only PR with the report. A Cursor Cloud automation or human reviewer should then run `agent-prompt.md` on that branch.

GitHub Actions acts as the sensor and optional agent runner. It should not silently fail just because a model secret is missing, because the discovery report is still useful intake for Cursor Automation or human review.
...
Validate deployable config files after an agent edits them:
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step requires repository secret `ANTHROPIC_API_KEY`. If that secret is absent, the workflow warns and opens a discovery-only PR for Cursor Automation or human review instead of failing before a PR exists.
