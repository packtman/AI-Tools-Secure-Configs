...
| `scripts/validate_config_files.py` | Repository-level validator for JSON, JSONC, YAML, TOML, and shell examples. |
...
7. If repository secret `ANTHROPIC_API_KEY` is available, the workflow runs the config-maintenance agent on the scoped tools, validates the edited files, commits the real config changes, and updates the PR.
8. If the secret is not available, the workflow still opens a discovery PR with the report so a human reviewer or Cursor Cloud automation can complete the config review.

The scanner can detect and stage the source-change signal. It should not decide the security posture for a brand-new vendor setting without an AI or human review step. Use the Cursor Cloud automation prompt in this directory if you prefer Cursor Cloud to produce the final config-update PR.
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step requires repository secret `ANTHROPIC_API_KEY`; if that secret is missing, the workflow skips the agent and still opens a report PR.
