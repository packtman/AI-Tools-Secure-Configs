...
7. If repository secret `ANTHROPIC_API_KEY` is available, the workflow runs the config-maintenance agent on the focused scope and validates any edited deployable config files before committing them.
8. If the secret is missing, the workflow still opens a discovery-only handoff PR for Cursor Automation or human review.
9. A Cursor Cloud automation can run `agent-prompt.md` on that branch. That agent reads the report, checks the upstream source, updates affected tiered configs and rollout docs, validates the files, commits the real config changes, and pushes the final PR branch.

GitHub Actions alone can detect and stage the source-change signal. It cannot safely decide the security posture for a brand-new vendor setting without an AI review step. Use the Cursor Cloud automation prompt in this directory for final config-update PR behavior when the GitHub agent secret is unavailable.
...
Validate only changed and untracked deployable config files before committing agent edits:
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step uses repository secret `ANTHROPIC_API_KEY` when present. If the secret is missing, the workflow opens a discovery-only PR instead of failing, so Cursor Automation or a human reviewer can complete the rollout-engineering update.
