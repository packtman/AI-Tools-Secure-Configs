...
| `.github/workflows/config-validation.yml` | PR and push workflow that validates scripts, source registry, and deployable configs. |
| `scripts/validate_config_files.py` | Local and workflow helper for validating deployable JSON, YAML, TOML, and shell files. |
...
7. If `ANTHROPIC_API_KEY` is available to GitHub Actions, the workflow runs the config-maintenance agent against `agent-scope.md`.
8. If the key is unavailable, the workflow opens a discovery handoff PR for Cursor Automation or human review instead of failing.
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step uses repository secret `ANTHROPIC_API_KEY` when available. If that secret is missing, the workflow still opens or updates the discovery PR so Cursor Automation or a human reviewer can process `agent-scope.md`.
