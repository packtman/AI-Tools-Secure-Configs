...
| `scripts/validate_config_files.py` | Repository-wide validator for deployable JSON, YAML, TOML, and shell examples. |
...
7. If `ANTHROPIC_API_KEY` is configured, the workflow runs the maintenance agent, validates edited config files, and commits focused config or documentation updates to the same branch.
8. If the key is missing or unavailable, the workflow still opens a discovery PR with the report and points reviewers to the Cursor Cloud automation prompt.

GitHub Actions alone can detect and stage the source-change signal. It cannot safely decide the security posture for a brand-new vendor setting without an AI review step. Use the Cursor Cloud automation prompt in this directory for final config-update PR behavior when the built-in maintenance agent is not available.
...
Validate deployable configuration files:
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step uses repository secret `ANTHROPIC_API_KEY` when it is available. If that secret is absent, the workflow opens a report-only PR so a Cursor Cloud automation or human reviewer can apply `agent-prompt.md`.
