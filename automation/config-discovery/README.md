...
7. If `ANTHROPIC_API_KEY` is available, the GitHub workflow runs the config-maintenance agent directly.
8. If the key is unavailable, the workflow still opens a discovery handoff PR for Cursor Automation or human review.
...
GitHub Actions can detect and stage the source-change signal. The AI review step decides the security posture for a brand-new vendor setting. Use the Cursor Cloud automation prompt in this directory as the fallback path when the repository secret is unavailable or when you prefer Cursor Cloud to produce the final config-update PR.
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step uses repository secret `ANTHROPIC_API_KEY` when available. If it is missing, the workflow skips the model step and opens a discovery handoff PR.
