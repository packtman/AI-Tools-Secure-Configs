...
| `.github/workflows/config-validation.yml` | Pull request validation for deployable JSON, YAML, TOML, shell, and discovery registry syntax. |
| `scripts/validate_config_files.py` | Local and CI validator for deployable config files. |
...
7. If `ANTHROPIC_API_KEY` is configured, the workflow runs the Claude Code action to review `agent-scope.md`, apply config changes, validate the files, and push the final PR branch.
8. If `ANTHROPIC_API_KEY` is not configured, the workflow still opens a discovery-only handoff PR for Cursor Automation or human review.
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step uses repository secret `ANTHROPIC_API_KEY` when it is available. If the secret is missing, the workflow opens a discovery-only handoff PR instead of failing before review.
