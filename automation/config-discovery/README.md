...
7. If repository secret `ANTHROPIC_API_KEY` is available, the workflow runs the config-maintenance agent, validates any generated config edits, commits them, and opens or updates the PR.
8. If the secret is unavailable, the workflow still opens a discovery-only PR. A Cursor Cloud automation or human reviewer should run `agent-prompt.md` on that branch to finish the config update.

GitHub Actions alone can detect and stage the source-change signal. It cannot safely decide the security posture for a brand-new vendor setting without an AI review step. Use the Cursor Cloud automation prompt in this directory when the repository does not have the optional model-backed workflow secret, or when you want Cursor Cloud to perform the final config-update review.
...
Validate all deployable config files after an agent or reviewer edits them:
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step uses repository secret `ANTHROPIC_API_KEY` when available. If the secret is missing, the workflow warns and opens a discovery-only PR instead of failing before reviewers see the report.
