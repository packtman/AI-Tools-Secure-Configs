...
| `scripts/validate_config_files.py` | Validates JSON, JSONC, YAML, TOML, and shell examples before agent commits. |
...
6. The workflow commits those files to a per-run `automation/config-maintenance-<run-id>-<attempt>` branch and opens a discovery branch.
7. If `ANTHROPIC_API_KEY` is available, the workflow runs the config-maintenance agent, validates changed config files, commits the real config changes, and pushes the final PR branch.
8. If `ANTHROPIC_API_KEY` is missing, the workflow opens a discovery-only handoff PR for Cursor Automation or a human reviewer instead of failing before a PR is created.

GitHub Actions alone can detect and stage the source-change signal. It cannot safely decide the security posture for a brand-new vendor setting without an AI review step. Use the Cursor Cloud automation prompt in this directory when the workflow opens a handoff PR.
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step uses repository secret `ANTHROPIC_API_KEY` when present. If the secret is absent, the workflow still opens a handoff PR.
