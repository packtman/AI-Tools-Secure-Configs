...
| `.github/workflows/config-discovery.yml` | Scheduled workflow that runs the scanner and opens or updates a config maintenance PR. |
| `.github/workflows/config-validation.yml` | Pull request workflow that validates deployable JSON, YAML, TOML, and shell examples. |
| `scripts/validate_config_files.py` | Reusable syntax validator used by workflows and local maintainers. |
...
7. If repository secret `ANTHROPIC_API_KEY` is available, the workflow runs the Claude Code maintenance agent against `agent-prompt.md`.
8. If the secret is unavailable, the workflow still opens a discovery PR with the report and scope file so a Cursor Cloud automation or human reviewer can finish the config review.
...
GitHub Actions acts as the sensor. The model-backed maintenance agent makes the security decision for a brand-new vendor setting. Use the Cursor Cloud automation prompt in this directory when the workflow cannot run the built-in agent or when you want a second scheduled reviewer.
...
Validate every deployable config example:

```bash
python3 scripts/validate_config_files.py --all
```

The validator checks `.json`, `.yaml`, `.yml`, `.toml`, and `.sh` files. It intentionally skips `.jsonc` because those files contain comments for documentation.
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step requires repository secret `ANTHROPIC_API_KEY`. If that secret is absent, discovery still succeeds and opens a PR for Cursor Automation or human review.
