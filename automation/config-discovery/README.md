...
| `build_agent_scope.py` | Builds `agent-scope.md` from missing-term sections in the report, grouped by unique tool. |
| `.github/workflows/config-discovery.yml` | Scheduled workflow that runs the scanner and opens or updates a PR. |
| `.github/workflows/config-validation.yml` | Pull request workflow that parses deployable JSON, YAML, TOML, and shell examples. |
...
7. If `ANTHROPIC_API_KEY` is available, the workflow runs the config-maintenance agent on the scoped report, validates any edited deployable configs, commits the real config changes, and opens or updates the PR.
8. If `ANTHROPIC_API_KEY` is missing, the workflow still opens a discovery handoff PR for Cursor Automation or human review instead of failing before a reviewer can see the upstream signal.

GitHub Actions can detect and stage the source-change signal. It should not silently invent a security posture for a brand-new vendor setting without an AI or human review step. Use the Cursor Cloud automation prompt in this directory as the fallback path when the GitHub-hosted agent cannot run.
...
No external package registry tokens or vendor API keys are required for discovery. The maintenance agent step requires repository secret `ANTHROPIC_API_KEY`. If that secret is not configured, the workflow opens a discovery-only handoff PR and asks Cursor Automation or a human reviewer to apply the scoped config update.
...
- Validates changed deployable config files with `scripts/validate_config_files.py --changed` before committing agent edits.
