...
def build_scope(report_text: str, max_tools: int) -> str:
    ...
    total_sources = sum(len(sources) for _, sources in tool_items)
    limited_sources = sum(len(sources) for _, sources in limited)
    lines.append(
        f"## Tools to process ({len(limited)} of {len(tool_items)} tools, {limited_sources} of {total_sources} sources with missing terms)"
    )
    lines.append("")
