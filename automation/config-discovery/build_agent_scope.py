...
def build_scope(report_text: str, max_tools: int) -> str:
    ...
    tools = list(sections_by_tool.items())
    ...
    for tool_name, source_entries in limited:
        ...
        lines.append("")
        lines.append("- Sources and missing terms:")
        for source_name, terms in source_entries:
            displayed_terms = ", ".join(f"`{term}`" for term in terms[:15])
            lines.append(f"  - {source_name}: {displayed_terms}")
            if len(terms) > 15:
