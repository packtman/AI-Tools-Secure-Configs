...
def build_scope(report_text: str, max_tools: int) -> str:
    ...
    grouped_by_tool: dict[str, list[tuple[str, list[str]]]] = {}
    ...
    for part in parts[1:]:
        ...
        if terms:
            if tool_name not in grouped_by_tool:
                grouped_by_tool[tool_name] = []
                grouped_sections.append((tool_name, grouped_by_tool[tool_name]))
            grouped_by_tool[tool_name].append((source_name, terms))
