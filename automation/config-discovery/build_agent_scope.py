...
def build_scope(report_text: str, max_tools: int) -> str:
    lines = [
        ...
        "5. Validate edited deployable files before finishing.",
        "",
    ...
    for tool_name, source_name, terms in extract_missing_sections(report_text):
    ...
    total_sources = sum(len(sources) for sources in grouped.values())
    lines.append(
        f"## Tools to process ({len(limited)} of {len(grouped_items)} tools with missing terms, {total_sources} source sections)"
    )
    lines.append("")
    ...
    if len(grouped_items) > max_tools:
        ...
        for tool_name, source_entries in grouped_items[max_tools:]:
