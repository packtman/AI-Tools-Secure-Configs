...
8. Validate every edited JSON, YAML, TOML, or shell file with `python3 scripts/validate_config_files.py --changed` or the file-specific commands in `AGENTS.md`.
...
- If the PR is a discovery handoff because `ANTHROPIC_API_KEY` was unavailable, finish the same review and validation steps that the inline maintenance agent would have performed.
