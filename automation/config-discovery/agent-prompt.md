...
8. Validate every edited JSON, YAML, TOML, or shell file with `python3 scripts/validate_config_files.py --changed` and the commands in `AGENTS.md`.
...
- If running on a discovery-only handoff PR, preserve the `automation/config-maintenance-<run_id>` branch and add actual config changes there.
