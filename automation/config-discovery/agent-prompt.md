...
8. Validate every edited JSON, JSONC, YAML, TOML, or shell file with `python3 scripts/validate_config_files.py` or the targeted commands in `AGENTS.md`.
...
- Leave file changes unstaged when running inside `.github/workflows/config-discovery.yml`; that workflow validates, commits, pushes, and opens the PR. Commit and push changes yourself only when running as a standalone Cursor automation.
