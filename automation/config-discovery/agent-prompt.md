...
8. Validate every edited JSON, YAML, TOML, or shell file with the commands in `AGENTS.md`, or run `python3 scripts/validate_config_files.py --changed origin/main` when available.
...
- Prefer `scripts/validate_config_files.py --changed origin/main` before committing when the helper script is present.
