...
8. Validate every edited JSON, YAML, TOML, or shell file with `python3 scripts/validate_config_files.py --changed` or the commands in `AGENTS.md`.
...
- If the workflow opened a discovery-only handoff PR because `ANTHROPIC_API_KEY` was unavailable, add the real config updates to that handoff branch rather than opening a separate report-only PR.
