...
8. Validate edited files with `python3 scripts/validate_config_files.py --changed-only`, plus the per-format commands in `AGENTS.md` if you need a narrower parser error.
...
- Run `python3 scripts/validate_config_files.py --changed-only` before committing.
