...
8. Validate every edited JSON, YAML, TOML, or shell file with `scripts/validate_config_files.py --changed` or the commands in `AGENTS.md`.
...
- Leave discovery-only PRs with clear "No config update needed" notes when every changed upstream term is documentation noise or already covered by local tier files.
