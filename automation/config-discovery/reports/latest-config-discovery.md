...
| Claude Desktop | Claude Desktop documentation | new-source-baseline | 200 | https://code.claude.com/docs/en/desktop.md |
| Claude Desktop | Claude Desktop local MCP support | new-source-baseline | 200 | https://support.claude.com/en/articles/10949351-getting-started-with-local-mcp-servers-on-claude-desktop |
...
No config update needed in this run: agent/view/artifact/away-summary/IDE-install controls are already covered by open PR #68. `ANTHROPIC_MODEL` remains intentionally unpinned (orgs choose models per provider).
...
No config update needed: model env vars are provider/org choices, not security defaults to pin in this repo.
...
No config update needed: subagent model selection is a cost/performance preference, not a threat-mitigation control for Baseline/Moderate/Strict.
...
"sha256:21ec0ec78ec9ce0de9dd962cf5ccb47c600051d44cc91535a4debf03010a4aa3",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1433,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:65312a0d3963dba7114ba33 ...
...
Security policy Activity Custom properties Stars 35.1k stars Watchers 165 watching Forks 5.1k forks
...
- Source URL: https://code.claude.com/docs/en/desktop.md
- Status: `200`
- Related repo paths: claude-desktop/

Keyword snippets:

> ... rop pane layout, integrated terminal and file editor, side chats, computer use, Dispatch
sessions from your phone, visual diff review, app previews, PR monitoring, connectors, and
enterprise configuration. The Claude Desktop app has three tabs: **Chat** for conversations,
**Cowork** for [Dispatch and longer agentic work](https://claude.com/product/cowork), and **Code**
for software ...

> ... ions added through the dialog are stored. #### Restrict which SSH hosts users can connect to
Administrators can limit Desktop's SSH sessions to an approved set of hosts by adding
`sshHostAllowlist` to a [managed settings](/docs/en/settings#settings-precedence) file. When set,
users can only connect to hosts whose resolved hostname matches one of the patterns. Set it to an e
...

> ... rganization already configured those lists for the extension, the Browser respects them
automatically. Administrators can also turn off Claude's tools on external pages with the
[`browserExternalPageTools` managed setting](#managed-settings). With tools disabled, users can
still navigate to external sites; Claude's tools can't read or act on them. To turn off external
browsing enti ...

> ... g](#managed-settings). With tools disabled, users can still navigate to external sites; Claude's
tools can't read or act on them. To turn off external browsing entirely, set the
[`disableBrowserExternalNavigation` managed setting](#managed-settings) to `true`. This blocks all
external navigation in the Browser, including sites on your organization's allowlist; localhost dev
servers and fil ...

> ... d or act on external pages in the [Browser pane](#browse-external-sites). Users can still
...
devices in the [iOS Simulator pane](/docs/en/desktop-ios-simulator#turn-off-simulator-access). The
pane stay ...

Potential config terms not found in local tool files:

`ANTHROPIC_MODEL`, `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING`, `CLAUDE_CODE_ENABLE_AUTO_MODE`, `allowManagedHooksOnly`, `allowManagedPermissionRulesOnly`, `autoMode`, `autoPort`, `autoVerify`, `availableModels`, `bypassPermissions`, `defaultMode`, `enabledPlugins`, `managedMcpServers`, `permissions.defaultMode`, `toolPolicy`

Review these terms first. If any are real admin controls, update the affected tier files and rationale docs.

Resolution in this run: added Desktop Browser, Simulator, SSH, and managedMcpServers controls to `claude-desktop/examples/config-*.json` and mirrored them in Claude Code managed settings. Remaining terms are either shared Claude Code controls already in `claude-code/` (`allowManagedHooksOnly`, `autoMode`, `bypassPermissions`) or developer-only preview knobs (`autoPort`, `autoVerify`, adaptive thinking) not required for tier hardening.

### Claude Desktop: Claude Desktop local MCP support

- Change type: `new-source-baseline`
- Source URL: https://support.claude.com/en/articles/10949351-getting-started-with-local-mcp-servers-on-claude-desktop
- Status: `200`
- Related repo paths: claude-desktop/

Keyword snippets:

> ... tes How to Get Support English Franais Deutsch Bahasa Indonesia Italiano   Portugus P  Espaol
English Search for articles... All Collections Claude Desktop Desktop extensions Getting Started
with Local MCP Servers on Claude Desktop Getting Started with Local MCP Servers on Claude Desktop
Updated over 3 weeks ago Table of contents The Model Context Prot ...

> ... olicy Controls Note: Enterprise policy controls at the user-machine level will override any in-
app controls (blocklist and allowlist). If you want to use an in-app control, ensure
isDesktopExtensionEnabled and isDesktopExtensionDirectoryEnabled are not set to "false" so the
allowlist can populate the available registry. For controlling desktop extensions through system
policies, ple ...

> ... cpb file. Follow the prompts to install and configure your custom desktop extension. For more
in-depth information, please refer to our desktop extension developer documentation . Enterprise
Policy Controls Note: Enterprise policy controls at the user-machine level will override any in-app
controls (blocklist and allowlist). If you want to use an in-app control, ensure isDes ...

> Getting Started with Local MCP Servers on Claude Desktop | Claude Help Center Skip to main content
API Docs Release Notes How to Get Support English Franais Deutsch Bahasa Indonesia Italiano
Portugus ...
...
No config update needed in this run: OpenAI Platform hosted-tool, network, and logging controls are already covered by open PR #69. Many `mcp_*` OpenAPI terms are Responses API event shapes, not org admin settings.
...
`fast-mode-2026-02-01`, `mcp-tunnels-2026-05-19`, `model_group`

Review these terms first. If any are real admin controls, update the affected tier files and rationale docs.

Resolution in this run: added `mcp_tunnels`, `fast_mode`, `rate_limit_groups.model_group`, and enterprise custom-role controls to Claude API tier policies. Prefer current tunnel beta `mcp-tunnels-2026-06-22`; keep `mcp-tunnels-2026-05-19` recorded as deprecated.
...
No config update needed for remaining terms: `LanguageModel` / `LanguageModelSession` are SDK types, `mcp_oauth` is client auth plumbing, and the others are API error/event names rather than deployable org policy knobs. Fast mode is covered in the tier files above.
