...
"sha256:e3087fb51aa487e2258138e9dcdb4b5a2c06e5e76a8bc9961222c7704d1bf4b3",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1421,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:f65f493018a9c8bf0e9e7d5 ...
...
management How can I delete or rename a conversation? Share and unshare chats ...
...
## Maintenance actions taken (2026-08-09)

### Claude Code: cross-session inbound and dialog expiry

Updated Moderate/Strict (and deployable `managed-settings.json` / rollout Moderate) with:

- `crossSessionInbound`: `"hold"` (Moderate) / `"refuse"` (Strict)
- `dialogExpiry`: `"5m"` (Moderate) / `"60s"` (Strict)
- Strict deny: `SendMessage`, `ListAgents`
- `minimumVersion`: `"2.1.224"` on Moderate/Strict
- Documented that `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS` overrides `dialogExpiry` and should not be set on managed fleets unless intentional

Baseline left unset so individuals can opt into peer messaging with vendor defaults.

### Discovery watcher repair: Codex Desktop

Replaced the moved `codex-rs/config.md` stub watcher with:

- `codex-rs/core/config.schema.json`
- `developers.openai.com/codex/config-advanced.md`
- `developers.openai.com/codex/enterprise/managed-configuration.md`

Registry: 14 tools / 31 sources. Consecutive live scan after refresh: no upstream source changes detected.

### No config update needed / deferred this run

- Claude Code IDE/agent/artifact/skill/autocompact env terms → already covered or open in #66/#68/#80/#81
- Codex CLI `ModelMessages` → release/internal naming, not an admin pin
- Codex Desktop new schema/advanced/managed baselines → fingerprinted only; plugin/approve-for-me pins deferred to avoid duplicating open #72/#77/#78/#79
- Claude Desktop MCP env names (`CLAUDE_CODE_MCP_SERVER_*`) → not Desktop admin controls
- OpenAI Platform schema MCP event terms → API protocol noise, not org policy knobs
- Claude API `fast-mode-2026-02-01` / Dreams / inference hooks → wait for org-level disable controls before pinning
