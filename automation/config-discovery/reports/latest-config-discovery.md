...
"sha256:6c93dd9c86006f0c945eb8d8c2d091d738a12a19c0fd779441b483c20ef65111",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1425,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:16d2bb7d0ac74ef5704ed04 ...
...
## Maintenance decisions (2026-07-27)

### Windsurf / Devin Desktop: config update applied
- Added MDM publisher-map keys (`MdmPolicies`), Linux deployable `mdm-policy-linux-*.json`, and Devin CLI team settings (MCP registry, sandbox enforcement, terminal permissions, Devin Local, Install CLI).
- Corrected Linux policy path to `/etc/windsurf/policies/policy.json`.
- Replaced noisy `docs.windsurf.com` watcher with official Devin enterprise policy and CLI team-settings docs.

### Codex Desktop: watcher repair only
- Upstream `codex-rs/config.md` moved to `docs/config.md`. Updated `tool-sources.json` to the new raw URL. No additional Codex Desktop tier edit in this run (open PR #69 already covers permission profiles / managed hooks).

### Claude Code: no config update in this run
- Missing env/managed terms (`CLAUDE_CODE_DISABLE_AGENT_VIEW`, artifact/bundled-skill/checkpoint/IDE controls, `CLAUDE_CODE_SUBAGENT_MODEL`) are already covered by open PRs #66 and #68. Avoided duplicate edits.

### Claude Desktop MCP env vars: no Desktop policy update
- `CLAUDE_CODE_MCP_*` / background-task env vars are Claude Code runtime controls, not Claude Desktop MDM keys. Open PR #70 covers Desktop Browser/SSH managed settings.

### OpenAI Platform: deferred
- OpenAPI `mcp_*` event names and related schema terms remain deferred to open PR #69 (hosted tool permissions and org policy coverage).

### Continue.dev / Copilot / Gemini / Tabnine / Amazon Q / Google Gemini: no config update needed
- Source fingerprints changed without new admin controls missing from local tier files for this run's unique scope.
