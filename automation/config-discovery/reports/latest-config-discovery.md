...
Copilot CLI Comparing CLI features Copilot CLI in Actions Cancel and roll back About remote control
Custom agents Autonomous task completion Parallel task execution Research ...
...
> ... emote/local versions. (#30297, #26705, #29375, #30981)\n- Codex can route authentication and
Responses API traffic through macOS and Windows system proxies, including PAC and WPAD
configurations. (#26708, #26709, #31335)\n- Added `codex remote-control pair` for generating manual
pairing codes from a running daemon. (#29913)\n- Added Amazon Bedrock GPT-5.6 Sol, Terra, and ...

> ... urn IDs, plus recommended remote-executor integration-test workflows. (#27714, #29790)\n\n##
Chores\n\n- Updated OpenSSL, Hono, fast-uri, quick-xml, and crossbeam-epoch to address security
advisories. (#29487, #29650, #30941, #31308)\n\n## Changelog\n\nFull Changelog:
https://github.com/openai/codex/compare/rust-v0.142.0...rust-v0.143.0\n\n- #26708 PAC 3 - Add Windo
...

> ... type": "application/octet-stream",         "digest":
"sha256:98eed03815ddfb1a551d4bd3dccc5d0580c9279f88d80c1b9a5d8a73894973bc",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1389,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:630cffeb5fa707c93659f68 ...

> ... 513 [codex] allow image generation with provider auth @richardopenai\n- #29526 core: resolve
view_image paths in selected environment @anp-oai\n- #29696 [codex] Fix stale approval policy in MCP
test @sayan-oai\n- #29704 [codex] Fix stale approval policy in MCP test @kumquatexpress\n- #29547
core: use current step environments for tools @sayan-oai\n- #28976 Add MCP ...
...
properties Stars 34.7k stars Watchers 165 watching Forks 5k forks Report reposit ...
...
> ... up, including on Windows. Fixed analytics connection failures under TLS-intercepting proxies.
Devin Local Edits produced in autonomous mode now produce reviewable diffs. Added an /mcp slash
command with a live MCP server status panel. ACU usage is now shown in the /usage command. Skill
permissions: frontmatter now applies to auto-approvals. Enterprise login pol ...

> ... dits produced in autonomous mode now produce reviewable diffs. Added an /mcp slash command with
a live MCP server status panel. ACU usage is now shown in the /usage command. Skill permissions:
...
output limits and warnings Raise the limit for a specific tool Tool input schemas ...
...
> ... 76 articles Pro and Max plans 15 articles Team and Enterprise plans 61 articles Identity
management (SSO, JIT, SCIM) 15 articles Claude Cowork 10 articles Claude Code 19 articles Claude
...
articles... Claude 76 articles Pro and Max plans 15 articles Team and Enterprise plans 61 articles
Identity management (SSO, JIT, SCIM) 15 articles Claude Cowork 10 articles Claude Code 19 articles
Claude Desktop 9 articles Claude Mobile apps 20 articles Claud ...
...
Resolution note (2026-07-08): `fast-mode-2026-02-01`, `mcp-tunnels-2026-05-19`, and `model_group` are API beta or integration terms, not durable organization policy keys. Claude API docs now explain that beta headers and model grouping choices should be governed through application change management, gateway allowlists, workspace RBAC, rate limits, and spend caps.
...
Resolution note (2026-07-08): `LanguageModel`, `LanguageModelSession`, `fast-mode-2026-02-01`, `mcp_oauth`, and `model_context_window_exceeded` are SDK, beta header, OAuth, or stop-reason terms. They do not map to this repo's Claude API tier policy JSON. The Claude API README and settings rationale now document how to govern these through application review, gateway policy, workspace rate limits, and spend caps.
