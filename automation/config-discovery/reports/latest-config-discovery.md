...
No config update needed: this fingerprint change is the watcher migration from rendered GitHub Docs HTML to GitHub's official raw Markdown source. The policy guidance is unchanged.
...
No config update needed: this fingerprint change is the watcher migration from rendered GitHub Docs HTML to GitHub's official raw Markdown source. Existing content-exclusion templates still match the documented path format and administration scope.
