...
| Claude Code | Corporate launcher documentation | new-source-baseline | 200 | https://code.claude.com/docs/en/corporate-launcher |
...
> ... ut Setup decision control InstructionsLoaded InstructionsLoaded input InstructionsLoaded
decision control UserPromptSubmit UserPromptSubmit input UserPromptSubmit decision control
UserPromptExpansion UserPromptExpansion input UserPromptExpansion decision control MessageDisplay
MessageDisplay input MessageDisplay output PreToolUse PreToolUse input PreToolUse decision control
De ...

> ... oaded decision control UserPromptSubmit UserPromptSubmit input UserPromptSubmit decision control
UserPromptExpansion UserPromptExpansion input UserPromptExpansion decision control MessageDisplay
MessageDisplay input MessageDisplay output PreToolUse PreToolUse input PreToolUse decision control
Defer a tool call for later PermissionRequest PermissionRequest input Permission ...
...
### Claude Code: Corporate launcher documentation

- Change type: `new-source-baseline`
- Source URL: https://code.claude.com/docs/en/corporate-launcher
- Status: `200`
- Related repo paths: claude-code/, rollout-guide/configs/claude-code/

Keyword snippets:

> ... her Copy page Copy page Route the processes Claude Code starts from its own binary, including
the background service and every agent view session, through a required launcher with
CLAUDE_CODE_PROCESS_WRAPPER or the processWrapper setting. Copy page Copy page Some organizations
require every process on a workstation to start through a mandatory launcher. The launcher applies
the sandbo ...

> ... rocesses Claude Code starts from its own binary, including the background service and every
agent view session, through a required launcher with CLAUDE_CODE_PROCESS_WRAPPER or the
processWrapper setting. Copy page Copy page Some organizations require every process on a
workstation to start through a mandatory launcher. The launcher applies the sandbox, network
controls, o ...

> Run Claude Code behind a corporate launcher - Claude Code Docs Documentation Index Fetch the
complete documentation index at: /docs/llms.txt Use this file to discover all available pages before
exploring further. Skip to ma ...

> ... s Deployment Run Claude Code behind a corporate launcher Copy page Copy page Route the processes
Claude Code starts from its own binary, including the background service and every agent view
session, through a required launcher with CLAUDE_CODE_PROCESS_WRAPPER or the processWrapper setting.
Copy page Copy page Some organizations require every process on a workstation ...

> ... hine where no new session starts keeps its unwrapped service until one does, and an installed
service always needs the restart in this step. 4 Verify Run /status in a session: the Self-exec
entry shows the resolved launch command and warns when the running background service doesn't match
it. claude daemon status prints the same information from the shell, including ...
...
"sha256:0f7969f154e7f7eda45b4ae95efa638714c0f25afc55628768c5580944c13496",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1421,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:fefea849a2dcf677d09025b ...
...
existing Compliance Access Key and the `read:compliance_user_data` scope. See [Retrieve local
sessions](/docs/en/manage-claude/compliance-content-data#retrieve-local-sessions). ### August 10,
2026 * The introdu ...
...
## Config update applied (2026-08-12)

Unique control focus for this run (deferred from prior maintenance notes):

- Claude Code: pin `sandbox.bwrapPath` / `sandbox.socatPath` and `wslInheritsWindowsSettings` on Moderate/Strict; bump `minimumVersion` to `2.1.210`.
- Document optional MDM-only `processWrapper` / `CLAUDE_CODE_PROCESS_WRAPPER` and `policyHelper` in `claude-code/examples/corporate-launcher-and-policy-helper.md` (not inventing fake launcher/helper paths in deployable JSON).
- Note `UserPromptExpansion` / `MessageDisplay` hook events for Strict managed-hooks coverage.
- Discovery: replace moved Codex Desktop `codex-rs/config.md` stub with schema + advanced + managed-configuration watchers; add Claude Code corporate-launcher and Claude API Inference hooks watchers. Registry 14 tools / 33 sources. Consecutive live scan: no upstream source changes detected.

Deferred duplicates remain in open PRs #66-#84 (Claude Code IDE/agent/autocompact/AskUserQuestion/peer/parentSettings/strictAllowlist/Inference hooks; Codex Desktop Apps/approvals; Continue; Gemini #64+#76). Next unique: reconcile Gemini after merge; Codex 0.147 Agent Plugins / `--approve-for-me` after #72; Dreams when an org kill switch ships.
