...
Scoped config work landed in the Claude Code tier files for `askUserQuestionTimeout`, `isolatePeerMachines`, `strictPluginOnlyCustomization`, and Remote Control push pins.

Deferred or non-actionable missing terms from this scan:

- Claude Code `ANTHROPIC_MODEL` / `CLAUDE_MODEL` / `CLAUDE_CODE_SUBAGENT_MODEL`: model routing preferences, not new admin deny/allow controls for this run. Keep org model allowlists via existing `availableModels` guidance when needed.
- Claude Code IDE/agent/artifact/checkpoint env names already covered by open PRs #66/#68; not duplicated here.
- Codex CLI `ModelMessages`: release/API noise, not a managed config key.
- Claude Desktop MCP env names and OpenAI Platform schema MCP event terms: deferred to a later unique run.
