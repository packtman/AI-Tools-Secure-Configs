...
> ... he confirmation dialog before [bypassPermissions mode](/docs/en/permission-modes#skip-all-
checks-with-bypasspermissions-mode) | Permission settings | User, local, or managed | |
[`skipWebFetchPreflight`](#skipwebfetchpreflight) | Skip the [WebFetch hostname
check](/docs/en/tools-reference#webfetch-tool-behavior) when Anthropic is unreachable | Privacy and
telemetry | Any file | ...
...
> ... onfirmation dialog before [bypassPermissions mode](/docs/en/permission-modes#skip-all-checks-
with-bypasspermissions-mode) | Permission settings | User, local, or managed | |
[`skipWebFetchPreflight`](#skipwebfetchpreflight) | Skip the [WebFetch hostname
check](/docs/en/tools-reference#webfetch-tool-behavior) when Anthropic is unreachable | Privacy and
telemetry | An ...
...
"sha256:ac8c27f8e9749811c9a35515924f08b3ee20ba79a1667ea2c5a239405f1e0d50",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1389,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:c2b170d4968b31bd3f5db8d ...
...
## Maintenance Decision (2026-09-03)

Applied this run:

- Claude Code `skipWebFetchPreflight: false` on Moderate and Strict (Baseline unset). Vendor default is unset, so the WebFetch hostname check already runs, but any user or project file can set `true` and skip Anthropic's blocklist. A managed `false` locks the check. Watcher: `code.claude.com/docs/en/settings-reference.md`.

No config update needed for scanner missing terms this run:

- `ANTHROPIC_MODEL` / `ANTHROPIC_DEFAULT_MODEL` / `ANTHROPIC_CUSTOM_MODEL_OPTION` are session model picks, not allowlists. Model allowlists are `availableModels` plus `enforceAvailableModels` (open PR #89).
- Remaining `CLAUDE_CODE_DISABLE_*` and `CLAUDE_CODE_ENABLE_*` env vars are one-session kills or local prefs for keys already in open PRs (#80 autocompact, #93 Fast mode, #98 Artifact, #68 agent view) or for UI/IDE convenience.
- `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS` is an operational stagger, not a security lock. `disableWorkflows` already covers workflows on Moderate/Strict.
- Codex CLI release names, Claude Desktop MCP server name/URL env vars, and OpenAI Platform schema fields are deferred (open PRs already cover nearby controls).

Did not add:

- `disableSideloadFlags` (open #61)
- `pluginSuggestionMarketplaces` (wait for #88)
- `managedSourcesBehavior` / `requiredMaximumVersion` / `httpHookAllowedEnvVars` (still deferred)
- `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS` (PR #67 called it conditional, not a tier default)
- Desktop Browser/Simulator keys (open #70)
- Codex 0.152 `tools.update_plan.enabled` (vendor default now off)
