...
| Claude Code | Channels documentation | new-source-baseline | 200 | https://code.claude.com/docs/en/channels.md |
...
No config update needed: these are session or environment model overrides, not an organization allowlist. Do not pin them in managed `env`. Use `availableModels` plus `enforceAvailableModels` when that control is approved (open PR #89). This run instead pins `allowedChannelPlugins` and `syncClaudeAiSkills`.
...
No config update needed: `ANTHROPIC_MODEL` and `CLAUDE_MODEL` are session model overrides. `CLAUDE_CODE_DISABLE_PERMISSION_PROMPT_NOTIFY_HOOKS` is a notification preference, not a permission or marketplace control.
...
No config update needed: `CLAUDE_CODE_SUBAGENT_MODEL` is a session model override. `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS` is a timing knob. Moderate and Strict already set `disableWorkflows: true`.

### Claude Code: Channels documentation

- Change type: `new-source-baseline`
- Source URL: https://code.claude.com/docs/en/channels.md
- Status: `200`
- Related repo paths: claude-code/, rollout-guide/configs/claude-code/

Keyword snippets:

> ... s by handle with `/imessage:access allow`. On top of that, you control which servers are enabled
each session with `--channels`, and your organization controls availability with
[`channelsEnabled`](#enterprise-controls) on claude.ai Team and Enterprise plans and on Console
organizations that deploy managed settings. Being in `.mcp.json` isn't enough to push messages: a
ser ...

> ... i Team and Enterprise: channels blocked. Console: channels allowed unless your organization
deploys managed settings, in which case channels are blocked until this key is set | |
`allowedChannelPlugins` | Which plugins can register once channels are enabled. Replaces the
Anthropic-maintained list when set. | Anthropic default list applies | Pro and Max users without an
organizat ...

> ... hey require Anthropic authentication through claude.ai or a Console API key, and are not
available on Amazon Bedrock, Google Cloud's Agent Platform, or Microsoft Foundry. Team and
Enterprise organizations must [explicitly enable them](#enterprise-controls). A channel is an MCP
server that pushes events into your running Claude Code session, so Claude can react to thin ...

> ... ersion`; if that fails, [install Bun](https://bun.sh/docs/installation). * **Team, Enterprise,
or managed Console org**: your admin must [enable channels](#enterprise-controls) in managed
settings Start a Claude Code session and run the install command: ```text theme={null} /plugin
install fakechat@claude-plugins-official ``` If the install fails, match the message Claude C ...

> ... n a fresh cloud session or wait to be polled, the event arrives in the session you already have
open: see [how channels compare](#how-channels-compare). You install a channel as a plugin and
configure it with your own credentials. Telegram, Discord, and iMessage are included in the research
preview. When Claude replies through a channel, you see the inbound messag ...
...
No config update needed for `MCP_PROTOCOL_NEGOTIATION`: it is a client-runtime protocol flag. Setting it to `auto` can prevent some channel servers from registering; it is not a substitute for `channelsEnabled` or `allowedChannelPlugins`. This run pins those two managed keys plus `syncClaudeAiSkills`.
...
"sha256:0db4b3bc6ab3adaa1d26c28392963d2b4d30389dba4f150085a76faded44a5dc",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1421,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:6c41eae71ed8047ac590061 ...
...
No config update needed: `CLAUDE_CODE_MCP_SERVER_NAME` and `CLAUDE_CODE_MCP_SERVER_URL` are Claude Code process environment labels, not Claude Desktop managed settings. Background-task and timeout variables are already covered by open PRs #63/#68 or are runtime knobs, not org allowlists.
