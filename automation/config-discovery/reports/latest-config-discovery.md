...
## Applied this run (2026-09-02)

Unique config update: pin Claude Code `attribution.sessionUrl: false` on Moderate and Strict (Baseline unset). Vendor default is `true`: cloud and Remote Control commits and PRs append a `Claude-Session` trailer and a claude.ai session link. Watcher added: `code.claude.com/docs/en/settings-reference.md`. Consecutive live scan: no further upstream source changes detected.

No additional config update from scanner missing terms in this run:
- `ANTHROPIC_MODEL` / `ANTHROPIC_DEFAULT_MODEL` / `ANTHROPIC_CUSTOM_MODEL_OPTION` are session model picks, not org allowlists (`availableModels` is already covered in open PR #89)
- `CLAUDE_CODE_AUTO_CONNECT_IDE` / `CLAUDE_CODE_IDE_SKIP_AUTO_INSTALL` / `CLAUDE_CODE_ENABLE_AWAY_SUMMARY` are local IDE/UI prefs, not managed threat controls
- Remaining `CLAUDE_CODE_DISABLE_*` env vars are one-session kills for keys already covered in open PRs (#61, #63, #68, #80, #93, #98) or deferred (`disableSideloadFlags` after #61, `pluginSuggestionMarketplaces` after #88, `managedSourcesBehavior`, HTTP hook allowlists, Codex 0.152 `tools.update_plan.enabled`)
...
> ... tiontimeout) | Let an unanswered question [auto-continue](/docs/en/tools-reference#question-
auto-continue-timeout) after idle time | Interface and terminal | User or managed | |
[`attribution`](#attribution) | Customize the attribution Claude Code adds to commits and pull
requests | Git and attribution | Any file | | [`attribution.commit`](#attribution-commit) | Change
...

> ... ttribution | Any file | | [`attribution.pr`](#attribution-pr) | Change or hide the attribution
line in pull request descriptions | Git and attribution | Any file | |
[`attribution.sessionUrl`](#attribution-sessionurl) | Omit the claude.ai session link from
[cloud](/docs/en/claude-code-on-the-web) and [Remote Control](/docs/en/remote-control) commits | Git
and attribut ...

> ... context | Any file | | [`cleanupPeriodDays`](#cleanupperioddays) | Choose how many days Claude
Code keeps [transcripts](/docs/en/data-usage#data-retention) before deleting them | Privacy and
telemetry | Any file | | [`companyAnnouncements`](#companyannouncements) | Show your organization's
announcements at startup | Interface and terminal | Any file | | [`crossSes ...

> ... link when it commits or opens a pull request from a [cloud](/docs/en/claude-code-on-the-web) or
[Remote Control](/docs/en/remote-control) session. Claude Code adds the link as a `Claude-Session`
trailer on commits and as a link in pull request descriptions. Set it to `false` to omit the link. *
**Scope**: [`Any file`](#scopes) * **Type**: Boolean * `true`: Claude Code ap ...
...
Security Self-Hosted Machines Settings API Origin Overview CLI Create a repository Clone, Push &
Pull Mirror GitHub Pull requests Browse & Search Settings Codebase settings ...
...
"sha256:e6730c5382e665433edd9796086ac6061d7f5ed133a803b00a41b316e37dc0a0",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1421,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:1f54025cc958ff1e937157a ...
...
Security policy Activity Custom properties Stars 35.7k stars Watchers 166 watching Forks 5.3k forks
