...
"sha256:9b366563f470af2694e83cd4f6f1140392d7871b19714695ff45f69a0029e608",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1389,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:231dcf4946b2ca9aa275e96 ...
...
> ... -mode tool guidance and executor exposure. features.enable_request_compression boolean Compress
streaming request bodies with zstd when supported (stable; on by default). features.fast_mode
boolean Enable model-catalog service tier selection in the TUI, including Fast-tier commands when
the active model advertises them (stable; on by default). features.goals boolean ...

> ... ures.fast_mode boolean Enable model-catalog service tier selection in the TUI, including Fast-
tier commands when the active model advertises them (stable; on by default). features.goals boolean
Enable persisted goals and automatic continuation (stable; on by default). features.hooks boolean
Enable lifecycle hooks loaded from hooks.json or inline [hooks] config. f ...

> ... environment to speed up repeated commands (stable; on by default). features.shell_tool boolean
Enable the default shell tool for running commands (stable; on by default).
features.skill_mcp_dependency_install boolean Allow prompting and installing missing MCP
dependencies for skills (stable; on by default). features.unified_exec boolean Use the unified PTY-
backed exec tool (stable; ena ...

> ... flags or --config overrides take precedence. An explicit model or reasoning-effort override
skips both managed model fields; service_tier is independent. Key Type / Values Details
allow_appshots boolean Set to false to disable Appshots for managed users. If omitted, Appshots
remain unconstrained by requirements and follow normal product availability. allow_login_shell boo
...

> ... aged_hooks_only boolean When true , Codex skips user, project, session, and plugin hooks while
still allowing managed hooks from requirements.toml and other managed config layers.
allow_remote_control boolean Set to false to disable device remote control for managed users. If
omitted, device remote control remains unconstrained by requirements and follows normal product
availab ...
...
Unique config update applied: Codex 0.149.0+ pins (`features.fast_mode`, `features.goals`, `features.skill_mcp_dependency_install`, `allow_appshots`, `allow_remote_control`, `allow_login_shell`) plus repair of the Codex Desktop config-reference watcher (moved off the dead `codex-rs/config.md` stub). Plugin catalog keys stay in open PR #87.

### Claude Code: Managed settings documentation

No config update needed: `ANTHROPIC_MODEL` / `ANTHROPIC_DEFAULT_MODEL` are session overrides. Org allowlisting is `availableModels` + `enforceAvailableModels` (open PR #89). Do not pin the env vars in managed `env`.

### Claude Code: Hooks documentation

No config update needed: `ANTHROPIC_MODEL` / `CLAUDE_MODEL` are session overrides (see #89). `CLAUDE_CODE_DISABLE_PERMISSION_PROMPT_NOTIFY_HOOKS=1` turns off Notification hooks for permission prompts in Desktop/VS Code hosts. Pinning it would weaken audit hooks, so it is left unset.

### Claude Code: Dynamic workflows documentation

No config update needed: `disableWorkflows` is already in the tier files. `CLAUDE_CODE_SUBAGENT_MODEL` is a session model override covered by the model allowlist. `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS` is a timing knob, not an admin security control.

### Claude Desktop: Claude Desktop MCP documentation

No config update needed: `CLAUDE_CODE_MCP_SERVER_*` and the other `CLAUDE_CODE_*` / `CLAUDE_AUTO_*` terms are Claude Code env vars, not Claude Desktop `claude_desktop_config.json` keys. Background-task and MCP idle controls belong in Claude Code (open PRs #63/#68), not Desktop MCP JSON.

### Continue.dev: Configuration reference

No config update needed: `mcpServers` coverage is in open PR #75.

### OpenAI Platform: OpenAI OpenAPI schema

Deferred: API schema names such as `mcp_call` and `allowed_tools` are Responses API event types, not org-policy keys. Revisit when OpenAI documents a new admin control.
