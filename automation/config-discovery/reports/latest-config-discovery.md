...
No config update is needed. The source change is the move from a volatile rendered page to GitHub's canonical raw documentation. Existing organization policy files already cover feature governance and organization-level enforcement.
...
No config update is needed. Existing tier files already exclude sensitive paths and document organization and repository deployment. This change establishes a stable raw source without changing the control's semantics.
...
No config update is needed. The rollout guide already tells administrators to alert on `copilot.content_exclusion_changed`. This new source adds a stable audit-specific watch so future event changes are detected directly.
