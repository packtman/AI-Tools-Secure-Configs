...
No config update needed: these are session model-selection env vars. The org lock is `availableModels` plus `enforceAvailableModels` (open PR #89). Do not pin `ANTHROPIC_MODEL` in managed `env` as a substitute.
...
No config update needed: `ANTHROPIC_MODEL` / `CLAUDE_MODEL` are covered by the model allowlist work in open PR #89. `CLAUDE_CODE_DISABLE_PERMISSION_PROMPT_NOTIFY_HOOKS` is a notify-hook UX flag, not an approval or sandbox control.
...
No config update needed: `CLAUDE_CODE_SUBAGENT_MODEL` is another model-selection env var (open PR #89). `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS` is a scheduling delay, not a security control. Workflows themselves stay governed by `disableWorkflows`.
...
#39864 Limit pending input preview wrapping work @copyberry\n- #39870 Allow session configuration
with `codex agents` @copyberry\n- #39873 Add keybindings for cycling TUI permission modes
@copyberry\n- #39876 Support attaching to existing realtime calls @copyberry\n- #39917 Ho ...

> ... type": "application/octet-stream",         "digest":
"sha256:71a62df83509924fe090e38dc79d2271eb9172e1e9846f88a0b81c85e2ec26b8",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1421,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:9e027de7e23c2475a775651 ...

> ... #40292 Add smoke tests for assembled Codex packages @copyberry\n- #40294 Classify internal model
context by source @copyberry\n- #40295 Classify permission instructions under the permissions
namespace @copyberry\n- #40296 Annotate Responses Lite base instructions @copyberry\n- #40297
Preserve developer instruction annotations in subagent forks @copyberry\n- #40301 Lab ...
...
> Configuration Reference | ChatGPT Learn For the complete documentation index, see llms.txt .
Markdown versions of documentation pages are available by appending .md to the page URL. Chat ...
...
> ... d-party integrations GitHub GitLab (Beta) Slack Linear Reference CLI customization Developer
commands Developer settings Overview Permissions Profiles Sandboxing Auto-review Agent approvals &
security Internet access Codex Security Overview Codex Security plugin Quickstart Run a security
scan Run a deep scan Review code changes Use the Security workbench Triage a ba ...

> ... le | keyring | auto Control where the CLI stores cached credentials (file-based auth.json vs OS
keychain). compact_prompt string Inline override for the history compaction prompt.
computer_use.windows.always_allowed_app_ids array<string> Windows app identifiers that Computer Use
can open without prompting. Apps not in the list require approval; remove saved entries from ...

> ... label string Display name shown in Open in menus. Required.
desktop.custom_file_handlers.<id>.supports_ssh boolean Offer the handler for files in SSH workspaces
(default: false ). developer_instructions string Additional developer instructions injected into the
session (optional). disable_paste_burst boolean Disable burst-paste detection in the TUI.
experimental_compact_prompt_fi ...
...
> ... Product checkout spec Guides UI guidelines Optimize Metadata Submit a Claude Code plugin
...
> ... l client, platform, and rollout availability. Restrict locked computer use To prevent Computer
Use from operating after a managed Mac locks, add this requirement: [ computer_use ]
allow_locked_computer_use = false This requirement doesn't enable Computer Use. It only prevents
locked use on macOS. If you omit it, requirements don't constrain locked use; normal product
availability an ...

> ... e flags for users receiving a managed requirements.toml : [ features ] personality = true
unified_exec = false # Disable surface-specific features when needed. browser_use = false
browser_use_full_cdp_access = false browser_use_external = false in_app_browser = false
in_app_updates = false computer_use = false Use the canonical feature keys from config.toml 's
[features] table for run ...

> ... d requirements.toml : [ features ] personality = true unified_exec = false # Disable surface-
specific features when needed. browser_use = false browser_use_full_cdp_access = false
browser_use_external = false in_app_browser = false in_app_updates = false computer_use = false Use
the canonical feature keys from config.toml 's [features] table for runtime features. The local runt
...

> ... erprise-managed requirements delivered in the cloud config bundle. Legacy managed_config.toml
fields that the local client reinterprets as requirements. macOS managed preferences (MDM) delivered
through com.openai.codex:requirements_toml_base64 . Higher-precedence layers override ordinary
scalar and list values from lower layers. Tables merge by key, while requ ...
...
Security policy Activity Custom properties Stars 35.7k stars Watchers 162 watching Forks 5.3k forks
...
No config update needed: `CLAUDE_CODE_MCP_SERVER_NAME` / `CLAUDE_CODE_MCP_SERVER_URL` are Claude Code runtime env vars, not Claude Desktop MDM keys. Background-task and experimental-beta env vars are Claude Code process flags. Desktop MCP allowlisting stays in `mcpServers`. This run pinned Codex 0.150 Computer Use and browser-debug requirements instead.
