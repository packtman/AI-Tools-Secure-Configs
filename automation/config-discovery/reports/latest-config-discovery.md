...
No config update needed: these are model/IDE UX environment variables, not organization managed-settings keys. Deferred to avoid duplicating open Claude Code PRs #66/#68.
...
"sha256:3b602a7f492d383ab8a4a68039b8ece038ba351abe73c5ac9829872d22e49979",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1421,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:8c47f7554558775b5acef21 ...
...
No config update needed: release-note runtime reconnect helpers, not `requirements.toml` admin pins. Codex CLI managed pins remain in open PR #72. This run covers Desktop `features.in_app_updates` instead.
...
> ... lean" }, "image_detail_original": { "type": "boolean" }, "image_generation": { "type": "boolean"
}, "imagegenext": { "type": "boolean" }, "in_app_browser": { "type": "boolean" }, "in_app_updates":
{ "type": "boolean" }, "item_ids": { "type": "boolean" }, "js_repl": { "type": "boolean" },
"js_repl_tools_only": { "type": "boolean" }, "local_thread_store_compression": { "typ ...
...
"Reviewer for approval prompts from this app, overriding the thread default." }, "default_ ...
...
Resolution in this run: pinned requirements-only `features.in_app_updates` across Baseline (`true`), Moderate (`false`), and Strict (`false`) Codex Desktop requirements, with rollout, tier delta, MDM, and CLI overlap notes. Replaced the moved `codex-rs/config.md` watcher with `config.schema.json` plus `config-advanced.md`. Left schema type names (`McpServerConfig`, `ModelProviderInfo`) and telemetry/metric keys unpinned. Deferred `auto_review` / `approvals_reviewer` and permission-profile terms to a later unique pass (avoid duplicating open Desktop PR #69).
...
> ... `status` | Startup prewarm duration by outcome. | | `startup_prewarm.age_at_first_turn_ms` |
histogram | `status` | Startup prewarm age when the first real turn resolves it. | |
`cloud_requirements.fetch.duration_ms` | histogram | | Workspace-managed cloud requirements fetch
duration. | | `cloud_requirements.fetch_attempt` | counter | See note | Workspace-managed cloud
requi ...

> ... ds project-scoped config files only when the project is trusted. If the project is untrusted,
Codex ignores project `.codex/` layers, including `.codex/config.toml`, project-local hooks, and
project-local rules. User and system layers remain separate and still load. Relative paths inside a
project config (for example, `model_instructions_file`) are resolved relat ...
...
No config update needed for remaining advanced-doc terms in this run: metric names (`mcp.call.*`, `remote_models.*`), SKU identifiers, and provider helper keys are not org-wide Desktop requirements pins. `guardian_policy_config` and `request_permissions` deferred pending stable managed-requirements examples beyond open PR #69.
