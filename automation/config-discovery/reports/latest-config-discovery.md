...
`ANTHROPIC_MODEL`, `CLAUDE_CODE_AUTO_CONNECT_IDE`, `CLAUDE_CODE_DISABLE_AGENT_VIEW`, `CLAUDE_CODE_DISABLE_BUNDLED_SKILLS`, `CLAUDE_CODE_ENABLE_AWAY_SUMMARY`, `CLAUDE_CODE_IDE_SKIP_AUTO_INSTALL`

Review these terms first. If any are real admin controls, update the affected tier files and rationale docs.

Review outcome: `disableAgentView` and `disableBundledSkills` were confirmed as real Claude Code managed settings and added to the tier files and rationale docs. `ANTHROPIC_MODEL`, `CLAUDE_CODE_AUTO_CONNECT_IDE`, `CLAUDE_CODE_ENABLE_AWAY_SUMMARY`, and `CLAUDE_CODE_IDE_SKIP_AUTO_INSTALL` are model, IDE, or local experience controls, not necessary hardened tier settings for this rollout.
...
No config update needed: `ANTHROPIC_MODEL` and `CLAUDE_MODEL` are model override environment variables, not organization security controls. Existing tier files already manage the risky execution surfaces that hooks can observe or block.
...
"sha256:0785762b1e6d36756f619b933b956dd005cb378b9e4f4f0d66599b607cff72c3",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1425,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:f641605152a86ef5a4c4f09 ...
...
properties Stars 33.7k stars Watchers 157 watching Forks 4.7k forks Report repos ...
...
> ... nt sign-in fails. Updated the bundled Devin Local agent to v2026.5.26-8 . Other improvements
Added .devinignore support alongside .windsurfignore and .codeiumignore . Settings and MCP
marketplace pages now scroll across the full panel width and keep content centered on wide panels.
Download 3.1.7  v3.0.28 June 6, 2026 Hardened migration from Windsurf on Window ...
...
No config update needed: these variables are passed to Claude Code MCP `headersHelper` scripts so one helper can serve multiple MCP servers. They are not Claude Desktop managed policy keys and should not be added to Desktop tier configs.
...
> ... les Pro and Max plans 15 articles Team and Enterprise plans 61 articles Claude API and Console
...
articles... Claude 84 articles Pro and Max plans 15 articles Team and Enterprise plans 61 articles
...
No config update needed: `model_group` is a rate-limit grouping field and `fast-mode-2026-02-01` is a beta header used for usage reporting and fast mode access. Neither is an organization policy key for the hardened Claude API examples.
...
No config update needed: `fast-mode-2026-02-01` is a beta header, `mcp_oauth` is a Claude Managed Agents credential type, and `model_context_window_exceeded` is a stop reason. These are API behavior terms, not deployable org policy settings.
