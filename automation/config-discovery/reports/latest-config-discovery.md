...
No config update needed: this change reflects switching the watcher from GitHub Docs HTML to the stable raw GitHub Docs markdown source. The policy content does not introduce a new local tier setting beyond the existing Copilot org policy examples.
...
No config update needed: this change reflects switching the watcher from GitHub Docs HTML to the stable raw GitHub Docs markdown source. Existing Copilot content exclusion examples already cover the watched content-exclusion behavior.
