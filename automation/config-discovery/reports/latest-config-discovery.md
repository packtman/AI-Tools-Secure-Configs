...
Config update applied: added tiered coverage for `disableAgentView`, `disableArtifact`, `disableBundledSkills`, `disableClaudeAiConnectors`, and `fileCheckpointingEnabled` in Claude Code managed settings, rationale docs, and rollout guidance. No config update needed for `ANTHROPIC_MODEL`, `CLAUDE_MODEL`, IDE auto-connect/install preferences, or away-summary preferences because they are model or developer workflow preferences rather than security controls for this repo's tiers.
...
No config update needed: `ANTHROPIC_MODEL` and `CLAUDE_MODEL` select model behavior for a session, but this repo does not pin tenant-specific model choices in hardened admin templates.
...
Config update applied: added explicit `mcpServers: {}` entries to Continue.dev strict, moderate, and baseline config templates, plus rationale and README guidance for approving scoped MCP servers later.
...
properties Stars 34.6k stars Watchers 164 watching Forks 4.9k forks Report repos ...
...
No Claude Desktop config update needed: these are Claude Code MCP helper/runtime environment variables, not Claude Desktop enterprise policy keys. Added Claude Desktop README and rationale notes so admins do not add unsupported keys to `claude_desktop_config.json`.
...
Documentation update applied: `model_group` is part of the Admin API rate-limit response shape, and `fast-mode-2026-02-01` is a premium usage beta header rather than an org policy key. Updated Claude API admin policy and rationale guidance instead of inventing tiered config keys.
...
> ... o Claude Code, see the [complete CHANGELOG.md](https://github.com/anthropics/claude-
code/blob/main/CHANGELOG.md) in the `claude-code` repository. ### June 26, 2026 * We've raised [rate
...
No tiered config update needed: `LanguageModel` and `LanguageModelSession` are SDK integration terms, `mcp_oauth` is a managed-agents vault credential type, and `model_context_window_exceeded` is an API stop reason. The actionable admin item in this section is fast mode cost governance, covered in the Claude API policy guidance update.
