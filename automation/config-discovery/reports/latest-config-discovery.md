...
| GitHub Copilot | Organization policy documentation | content-changed | 200 | https://docs.github.com/api/article/body?pathname=/en/copilot/how-tos/administer-copilot/manage-for-organization/manage-policies |
| GitHub Copilot | Content exclusion documentation | content-changed | 200 | https://docs.github.com/api/article/body?pathname=/en/copilot/how-tos/configure-content-exclusion/exclude-content-from-copilot |
...
- Change type: `content-changed`
- Source URL: https://docs.github.com/api/article/body?pathname=/en/copilot/how-tos/administer-copilot/manage-for-organization/manage-policies
- Status: `200`
...
- Change type: `content-changed`
- Source URL: https://docs.github.com/api/article/body?pathname=/en/copilot/how-tos/configure-content-exclusion/exclude-content-from-copilot
- Status: `200`
- Related repo paths: github-copilot/, rollout-guide/configs/github-copilot/
...
"sha256:760b13f039f9550294c30d4b2c976365987014d9af4dd026227e034daa699407",         "label": "",
"name": "codex-windows-sandbox-setup",         "size": 1389,         "state": "uploaded"       },
{         "content_type": "application/x-msdos-program",         "digest":
"sha256:43e51904c21a6c702b56960 ...
...
Security policy Activity Custom properties Stars 35.2k stars Watchers 164 watching Forks 5.1k forks
...
## Maintenance agent decisions (2026-07-29)

### Codex CLI

Applied config updates for Codex `rust-v0.146.0` managed controls:
- Added `examples/requirements-{baseline,moderate,strict}.toml` with exact managed pins for `allow_login_shell`, `check_for_update_on_startup`, `windows.sandbox_private_desktop`, preview MCP (`mcp_2026_07_28`, `enable_mcp_apps`), skill MCP auto-install, remote plugins, apps, and related feature flags.
- Mirrored pins into `config-*.toml` and `system-config.toml`.
- Documented rollout plan, tier delta, deployment paths, SIEM, and workflow-preservation notes in `codex-cli/README.md`.

No config update needed for `McpConnectionSet`, `McpRuntime`, or `forceRefetch`. These appear in release notes as internal MCP runtime/refetch implementation names, not documented admin settings in the Codex configuration reference.

### GitHub Copilot

Applied config updates for the enterprise **agent apps** policy (public preview): set `agent_policies.agent_apps` to `disabled` on Baseline, Moderate, and Strict templates, with rationale and overlap notes for third-party MCP hosts.

### Claude Code

No config update needed in this PR. Missing terms (`CLAUDE_CODE_DISABLE_AGENT_VIEW`, artifact/bundled-skill/checkpoint/IDE controls, `CLAUDE_CODE_SUBAGENT_MODEL`) are already covered by open PRs #66 and #68. Avoided duplicate edits.

### Claude Desktop / OpenAI Platform

Deferred. Claude Desktop background-task env terms overlap Claude Code runtime docs; OpenAI Platform OpenAPI MCP/retention terms are covered by open PR #69.

### Discovery watcher repairs

- Cursor watchers moved to `cursor.com/docs/enterprise` and `cursor.com/docs/mcp`.
- Copilot watchers moved to GitHub Docs markdown API endpoints.
- Codex Desktop/CLI config watchers moved to `developers.openai.com/codex/config-reference` (repo `codex-rs/config.md` is now a stub).
