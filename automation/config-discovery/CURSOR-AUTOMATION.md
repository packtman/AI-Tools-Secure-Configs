...
GitHub Actions acts as the sensor. Cursor Cloud can act as the config-maintenance agent, especially when the repository does not configure `ANTHROPIC_API_KEY` for the in-workflow agent.

## Recommended Trigger

- Schedule: daily, after `.github/workflows/config-discovery.yml` has run.
- Repository: this repo.
- Branch: `automation/config-maintenance` when an open discovery PR exists, otherwise the default branch.
...
2. Read automation/config-discovery/reports/agent-scope.md first. Process only the scoped tools for this run.
3. Read automation/config-discovery/reports/latest-config-discovery.md for source URLs and snippets for the scoped tools.
