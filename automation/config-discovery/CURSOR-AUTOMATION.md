...
GitHub Actions acts as the sensor. Cursor Cloud acts as the config-maintenance agent when the GitHub-hosted Claude Code action cannot run, for example when `ANTHROPIC_API_KEY` is not available to the workflow.

## Recommended Trigger

- Schedule: daily, after `.github/workflows/config-discovery.yml` has run.
- Repository: this repo.
- Branch: the latest `automation/config-maintenance-<run_id>` PR branch when GitHub Actions opened a handoff PR, otherwise the default branch.
...
8. Do not create a report-only PR when a relevant control changed. The PR should contain actual config changes.
...
10. Validate edited JSON, YAML, TOML, and shell files with `python3 scripts/validate_config_files.py --changed` and the commands in AGENTS.md.
