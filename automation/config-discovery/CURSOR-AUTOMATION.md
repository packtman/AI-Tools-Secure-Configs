...
1. Check for an open config maintenance PR or branch named automation/config-maintenance. It may be discovery-only if the GitHub workflow did not have ANTHROPIC_API_KEY.
...
10. Validate edited JSON, YAML, TOML, and shell files with python3 scripts/validate_config_files.py --changed-only.
