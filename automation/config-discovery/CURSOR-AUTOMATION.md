...
- Schedule: daily, after `.github/workflows/config-discovery.yml` has run, or on new PRs whose branch starts with `automation/config-maintenance-`.
- Repository: this repo.
- Branch: the open config-maintenance PR branch when one exists, otherwise the default branch.
...
1. Check for an open config maintenance PR whose branch starts with automation/config-maintenance-.
2. Read automation/config-discovery/reports/agent-scope.md first. Process only the tools listed there.
3. Read automation/config-discovery/reports/latest-config-discovery.md for details on those scoped tools.
