...
GitHub Actions acts as the sensor. Cursor Cloud can act as the config-maintenance agent when the GitHub workflow cannot run its model-backed agent or when you prefer Cursor to make the config edits.
...
11. Validate edited JSON, YAML, TOML, and shell files with `python3 scripts/validate_config_files.py --changed --base origin/main`.
