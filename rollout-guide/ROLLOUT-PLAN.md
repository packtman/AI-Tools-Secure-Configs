...
- Claude Code cloud and Remote Control commits will not include a `Claude-Session` trailer or a session link in the PR description. Keep the session ID in your private notes or the session UI if you need it for debugging.
...
| `attribution.sessionUrl` | Unset (vendor default `true`) | `false` | `false` | Moderate and Strict omit claude.ai session links from cloud and Remote Control commits and PRs; Baseline keeps the vendor default for local debugging |
...
# Verify session-link attribution is off (cloud or Remote Control commit)
git log -1 --format=%B | grep -i Claude-Session
# Expected: no match. If a trailer appears, the managed attribution.sessionUrl pin is not active.
```
...
| Claude Code `Claude-Session` trailer / PR session link | Session URLs in git history and PR text can leak that a change came from a Claude session and can expose session IDs | Keep the session ID in your private notes or the Claude Code session UI. Link the PR to an internal ticket instead. | Claude Code |
...
| `attribution.sessionUrl: false` | Developer wants the claude.ai session link on cloud or Remote Control commits for debugging | Omit the key from that group's managed payload so the vendor default (`true`) applies. Keep session IDs in private notes instead of git. Do not pin `attribution.commit` or `attribution.pr` as a substitute. |
...
| **Commit attribution** | `attribution.sessionUrl` covers Claude Code cloud and Remote Control commits only. Cursor commit generation and GitHub Copilot PR summaries are separate. Configure those tools independently if you also need to keep session or tool identifiers out of git. |
