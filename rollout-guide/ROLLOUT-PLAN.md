...
- Claude Code WebFetch still asks before fetching, and it also checks each hostname with Anthropic first. Do not add `skipWebFetchPreflight: true` in a user or project file.
...
| `skipWebFetchPreflight` | Unset (vendor default: check runs) | `false` | `false` | Moderate and Strict lock the WebFetch hostname safety check so a user or project file cannot skip Anthropic's blocklist; Baseline keeps the vendor default |
...
# Verify WebFetch preflight is locked on (read the OS path from the table above)
python3 -c "import json,sys; print(json.load(open(sys.argv[1])).get('skipWebFetchPreflight'))" /path/to/managed-settings.json
# Expected: False. If the key is missing or true, a user or project file can skip Anthropic's hostname blocklist.
```
...
| `skipWebFetchPreflight: false` | Bedrock, Vertex, or Foundry egress blocks `api.anthropic.com`, so WebFetch hostname checks fail | Omit the key from that group's managed payload. Set `true` only for those providers, and keep `WebFetch` in `ask` or `deny`. Do not skip the check on Anthropic-hosted Claude Code. |
...
| **WebFetch hostname check** | `skipWebFetchPreflight` covers Claude Code WebFetch only. Cursor browser tools and GitHub Copilot web search are separate. Configure those tools independently if you also need to keep hostname or domain checks on. |
