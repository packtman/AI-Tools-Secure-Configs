...
1. **Claude Code**: Write, edit, and shell commands now require your approval before running. You will see a prompt asking "Allow this action?" Read-only operations (searching, reading files, listing directories) still run automatically. Dynamic workflows and background agents are disabled in the Moderate tier until IT completes a pilot for long-running agent work. Artifact publishing and automatic claude.ai MCP connectors are disabled until data handling and connector access reviews are complete.
...
- Claude Code workflow commands, workflow keyword triggers, ultracode, background agents, and agent view commands are unavailable in the Moderate tier.
- Claude Code cannot publish artifacts to claude.ai or automatically connect claude.ai MCP connectors in the Moderate tier.
...
| `disableAgentView` | `false` | `true` | `true` | Moderate and Strict block background agents until admins define pilot groups, repository scope, and monitoring |
| `disableArtifact` | `false` | `true` | `true` | Moderate and Strict prevent session output from being published to claude.ai before data handling review |
| `disableBundledSkills` | `false` | `false` | `true` | Strict exposes only explicitly reviewed skills; Moderate keeps bundled skills for productivity because tool approvals still apply |
| `disableClaudeAiConnectors` | `false` | `true` | `true` | Moderate and Strict require cloud MCP connector inventory and access review before enabling connectors |
| `fileCheckpointingEnabled` | `true` | `true` | `false` | Strict minimizes extra local copies of edited files; Baseline and Moderate preserve quick rollback with `/rewind` |
...
| `minimumVersion` | `2.1.38` | `2.1.182` | `2.1.182` | Moderate and Strict need v2.1.182 or later for `disableClaudeAiConnectors`; Baseline only needs the server-managed settings floor |
...
3. Requires Claude for Teams or Enterprise plan, Claude Code >= 2.1.182 for this Moderate policy
...
| Claude Code background agents and agent view | Background work can continue outside the immediate human review loop | Use a normal foreground Claude Code session. Request a pilot exception for monitored background-agent use. | Claude Code |
| Claude Code artifact publishing | Session output can include source code, file paths, incident notes, or architecture details | Share through approved internal systems such as your wiki, ticket tracker, or secure paste service. | Claude Code |
| Automatic claude.ai MCP connectors | Connectors can expose external systems before IT reviews data access and authentication scopes | Request an approved MCP connector entry with scoped credentials, version pinning where applicable, and SIEM coverage. | Claude Code |
...
| `disableAgentView: true` | Developer wants background agents for a large refactor | Treat this like dynamic workflows. Approve only for pilot groups with repository scope, usage monitoring, and clear owner accountability. |
| `disableArtifact: true` | Developer wants to share a generated explanation outside the terminal | Use approved internal collaboration tools. If artifacts are needed, review retention, access control, and whether source code can appear in output. |
| `disableClaudeAiConnectors: true` | Developer wants a claude.ai connector for a SaaS tool | Require connector review, least-privilege OAuth scopes, and an audit log destination before enabling. |
