...
| `disableAgentView` | `false` | `true` | `true` | Background agents are blocked in enterprise tiers until cost, audit, and support controls are ready |
| `disableArtifact` | `false` | `true` | `true` | Enterprise tiers keep session output local until sharing and retention rules are approved |
| `disableBundledSkills` | `false` | `false` | `true` | Moderate keeps bundled skills for workflow compatibility; Strict requires explicitly approved skills |
| `disableClaudeAiConnectors` | `false` | `true` | `true` | Enterprise tiers block unmanaged claude.ai MCP connectors |
| `fileCheckpointingEnabled` | `true` | `true` | `false` | Baseline and Moderate preserve `/rewind`; Strict minimizes local edit snapshots |
...
| `disableAgentView: true` | Developer wants background agents or Agent View | Treat this as an automation exception. Approve only for pilot groups with cost monitoring, session ownership, and SIEM audit coverage. |
| `disableArtifact: true` | Developer wants to share Claude Code session output through claude.ai artifacts | Use the normal code review, ticket, or internal documentation system until artifact retention and sharing rules are approved. |
| `disableClaudeAiConnectors: true` | Developer wants a claude.ai cloud connector | Route the request through MCP exception review. Approve only after data scope, authentication, and audit logging are documented. |
