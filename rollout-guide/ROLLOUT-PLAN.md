...
- Claude Code workflow commands, workflow keyword triggers, ultracode, background agents, and the agent view are unavailable in the Moderate tier.
...
| `disableAgentView` | `false` | `true` | `true` | Baseline allows local background-agent experimentation; Moderate and Strict block unattended background agents until admins pilot audit and exception controls |
...
| Claude Code background agents | Unattended agents can continue editing files or requesting tools while the developer is away | Use a normal foreground Claude Code session. Request a pilot exception if your team needs `claude agents`, `--bg`, `/background`, or agent view. | Claude Code |
...
| `disableAgentView: true` | Developer wants Claude Code to run background agents while they do other work | Treat this as an exception request. Approve only for pilot groups with audit coverage, repository scope, and usage-spend monitoring. |
